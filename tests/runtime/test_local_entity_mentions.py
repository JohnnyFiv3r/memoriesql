...
class LocalMentions(fixtures.SourceRevisiting):
    ...
    def setup_mentions(self) -> None:
        ...
        self.assertEqual(
            self.complete.activate_mentions(command).execution_task_id,
    ...
    def test_ambiguous_local_mention_is_atomic_with_accepted_receipt(self) -> None:
        ...
        self.assertEqual(
            self.row(
                "SELECT surface_text,local_identity_state,local_identity_reason,start_offset,end_offset FROM memoriesql.entity_mentions"
            ),
            (
                "Alex",
                "ambiguous",
                self.mentions[0]["local_identity_reason"],
                None,
                None,
            ),
        )
        self.assertEqual(
            self.row("SELECT count(*) FROM memoriesql.entity_mention_resolutions"), (0,)
        )
        self.assertEqual(
            self.row(
    ...
    def test_aggregate_payload_bound_rejects_individually_valid_mentions(self) -> None:
        self.mentions = [
            dict(
                entity_mention_id=str(uuid.uuid4()),
                surface_text="🌳" * 1024,
                local_identity_state="unresolved",
                local_identity_reason=None,
            )
            for _ in range(32)
        ]
        self.setup_mentions()
    ...
    def test_authored_run_reference_is_rebound_before_apply(self) -> None:
        # A live model bound statements to the task kind; the executor now binds
        # them to its own run reference before hashing, so apply succeeds.
        self.setup_mentions()
        ...
        def author(messages: Any, info: Any) -> Any:
            ...
            part = cast(ToolCallPart, response.parts[0])
            data = part.args_as_dict()
            for statement in data["typed_output"]["annotations"][0]["statements"]:
                statement["model_run_ref"] = "memory.semantic.author-complete-unit"
            return ModelResponse(
                parts=[ToolCallPart(part.tool_name, data)], usage=response.usage
            )
        ...
        self.assertEqual(
            self.row(
                "SELECT DISTINCT semantic_run_id FROM memoriesql.bead_semantic_statements"
            ),
            ("orchard.complete.run",),
        )

    def test_canonical_apply_refusal_settles_the_attempt(self) -> None:
        # A refused apply used to raise through cleanup and leave the attempt
        # running for the reaper; it now settles as invalid output.
        ...
        def refuse(queue: Any, fence: Any, result: Any, *, recorded_at: Any) -> str:
            raise psycopg.errors.InvalidParameterValue(
                "semantic statement run is unavailable"
            )
        ...
        with patch.object(PostgresSemanticTaskQueue, "record_canonical_result", refuse):
            receipt = asyncio.run(cycle())
    ...
    def test_sql_failure_rolls_back_mentions_and_replay_preserves_set(self) -> None:
        ...
        def intercept(queue: Any, fence: Any, result: Any, *, recorded_at: Any) -> str:
            ...
            command = dict(
                ...
                tenant_id=str(fence.tenant_id),
                workspace_id=str(fence.workspace_id),
                access_scope_id=str(fence.access_scope_id),
                ...
                model_run_refs=list(result.model_run_refs),
                payload=payload,
            )
            ...
            with (
                owner.assertRaises(psycopg.errors.UniqueViolation),
...
def load_tests(loader: Any, standard_tests: Any, pattern: Any) -> Any:
    ...
    return unittest.TestSuite(
        LocalMentions(name)
        for name in LocalMentions.__dict__
