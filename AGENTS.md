...
- The approved migration substrate is limited to the explicit migration inventory, runner, schema inspection and resource closure. Preserve SQL 0001–0015 byte-for-byte. The runtime cut is limited to `contracts/runtime-inventory.json`: neutral runtime and internal static mechanics, with caller-supplied composition. The public-authored schema-15 transition adds only the inventoried immutable-observation commands, task definitions and adapter dispatch. The public-authored schema-16 transition adds only version-1 complete-input evidence package storage and bounded authorized reads; see `docs/evidence-packages.md`. No semantic execution/materialization or production provider binding is added. No supported plugin platform is included.
