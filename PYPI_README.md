...
It preserves the 49 published provider-neutral records: 43 JSON Schema records, five Python-surface records, and one synthetic reference connector. Current unreleased development adds evidence-package and logical-unit-materialization v1 contracts (51 records, including 45 JSON Schema records). Materialized complete-input tasks remain unavailable for execution. Five additional catalog kinds are present as explicit empty `not_implemented` documents. Twelve experimental Python and CLI APIs provide catalog listing and exact record lookup.
