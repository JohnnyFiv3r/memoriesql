...
Published 0.0.10 includes schemas 1–26: stored-result inspection, the explicit
hard-bounded model-admission interface, declared evidence scopes (schema 25),
[supervised managed dispatch](supervised-dispatch.md) (schema 26), the
terminal-settlement repair and the run-reference binding. The prepared 0.0.11
candidate adds [authored claims and relations](authored-claims-and-relations.md)
(schema 27, author-complete-unit revision 6); publication remains gated. Installing or upgrading does not provision trust,
