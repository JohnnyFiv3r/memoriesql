-- Forward opt-in revision 3; prior tasks, receipt payloads and policies retain their meaning.
ALTER TABLE memoriesql.complete_input_executions
 ADD COLUMN execution_contract_revision integer NOT NULL DEFAULT 2 CHECK(execution_contract_revision IN (2,3)),
 ADD COLUMN authorized_context jsonb NOT NULL DEFAULT '[]' CHECK(jsonb_typeof(authorized_context)='array' AND jsonb_array_length(authorized_context)<=4);
ALTER TABLE memoriesql.complete_input_dispatch_policies
 ADD COLUMN execution_contract_revision integer NOT NULL DEFAULT 2 CHECK(execution_contract_revision IN (2,3));
ALTER TABLE memoriesql.complete_input_dispatch_receipts
 ADD COLUMN delivery_version integer NOT NULL DEFAULT 1 CHECK(delivery_version IN (1,2)),
 ADD COLUMN selection jsonb,
 ADD COLUMN delivered_units integer NOT NULL DEFAULT 0 CHECK(delivered_units BETWEEN 0 AND 65536);

-- Recovery's clock-aware policy check is reused in meaning, without changing any legacy reader.
CREATE FUNCTION memoriesql.revisiting_source_authorize(source_id uuid) RETURNS memoriesql.source_objects
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
DECLARE s memoriesql.source_objects%ROWTYPE; c memoriesql.authorization_contexts%ROWTYPE; until_time timestamptz;
BEGIN
 ...
 JOIN memoriesql.workspace_memberships m ON m.role_key=cap.role_key
 WHERE m.tenant_id=c.tenant_id AND m.workspace_id=c.workspace_id AND m.principal_id=c.principal_id
 ...
 s:=memoriesql.evidence_package_authorize(source_id,false);
 SELECT LEAST(c.expires_at,a.expires_at) INTO until_time FROM memoriesql.authentication_credentials a
 WHERE a.tenant_id=c.tenant_id AND a.credential_id=c.credential_id AND a.principal_id=c.principal_id AND a.status='active';
 IF until_time IS NULL OR until_time<=clock_timestamp()
 OR NOT memoriesql.current_context_scope_time_authorized(s.access_scope_id,'read',clock_timestamp()) THEN
  RAISE EXCEPTION 'source_revisiting_authority_unavailable' USING ERRCODE='42501'; END IF;
 RETURN s;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.revisiting_source_authorize(uuid) FROM PUBLIC;

CREATE FUNCTION memoriesql.revisiting_context_valid(t uuid, workspace uuid, scope uuid, target uuid, contexts jsonb) RETURNS boolean
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql AS $$
DECLARE item jsonb; p memoriesql.evidence_packages%ROWTYPE; s memoriesql.source_objects%ROWTYPE; seen uuid[]:=ARRAY[target];
BEGIN
 IF jsonb_typeof(contexts) IS DISTINCT FROM 'array' OR jsonb_array_length(contexts)>4 THEN RETURN false; END IF;
 FOR item IN SELECT value FROM jsonb_array_elements(contexts) LOOP
  IF item-ARRAY['package','source_object_id','source_schema_version']<>'{}'::jsonb OR (item#>>'{package,package_id}')::uuid=ANY(seen) THEN RETURN false; END IF;
  SELECT * INTO p FROM memoriesql.evidence_packages WHERE tenant_id=t AND package_id=(item#>>'{package,package_id}')::uuid;
  s:=memoriesql.revisiting_source_authorize(p.source_object_id);
  IF p.workspace_id IS DISTINCT FROM workspace OR p.access_scope_id IS DISTINCT FROM scope OR p.sealed_receipt_id IS NULL
   OR p.source_object_id::text IS DISTINCT FROM item->>'source_object_id' OR s.schema_version::text IS DISTINCT FROM item->>'source_schema_version'
   OR item->'package' IS DISTINCT FROM jsonb_build_object('package_id',p.package_id,'sealed_receipt_id',p.sealed_receipt_id,
     'inventory_sha256',p.inventory_hash,'required_parts',p.part_count,'required_characters',p.character_count,
     'required_utf8_bytes',p.utf8_byte_count,'coverage','entire_sealed_inventory') THEN RETURN false; END IF;
  seen:=array_append(seen,p.package_id);
 END LOOP;
 RETURN true;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.revisiting_context_valid(uuid,uuid,uuid,uuid,jsonb) FROM PUBLIC;

CREATE FUNCTION memoriesql.source_revisiting_authorize(t uuid, task uuid, selected uuid DEFAULT NULL) RETURNS void
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
...
BEGIN
 ...
 END LOOP;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.source_revisiting_authorize(uuid,uuid,uuid) FROM PUBLIC;

-- A typed read is selection/hydration only. It never creates exposure.
CREATE FUNCTION memoriesql.source_revisiting_read(t uuid, task uuid, request jsonb) RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql AS $$
DECLARE e memoriesql.complete_input_executions%ROWTYPE; b memoriesql.logical_unit_materializations%ROWTYPE;
...
BEGIN
 ...
 PERFORM memoriesql.source_revisiting_authorize(t,task,(request->>'package_id')::uuid);
...
END; $$;
REVOKE ALL ON FUNCTION memoriesql.source_revisiting_read(uuid,uuid,jsonb) FROM PUBLIC;

CREATE OR REPLACE FUNCTION memoriesql.activate_complete_input_v1(request jsonb) RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
...
BEGIN
    ...
    IF FOUND THEN
        IF e.execution_contract_revision<>2 OR e.dispatch_policy_id<>(request->>'dispatch_policy_id')::uuid THEN RAISE EXCEPTION 'complete_input_activation_conflict' USING ERRCODE='23505'; END IF;
...
CREATE OR REPLACE FUNCTION memoriesql.activate_complete_input_v2(request jsonb) RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
DECLARE c memoriesql.authorization_contexts%ROWTYPE; b memoriesql.logical_unit_materializations%ROWTYPE;
    old memoriesql.idempotency_receipts%ROWTYPE; e memoriesql.complete_input_executions%ROWTYPE;
    original memoriesql.semantic_tasks%ROWTYPE; p memoriesql.evidence_packages%ROWTYPE;
    rid uuid:=uuidv7(); tid uuid:=uuidv7(); eid uuid; input jsonb; result jsonb; request_hash text; started timestamptz:=clock_timestamp();
BEGIN
    IF request->>'contract_version' IS DISTINCT FROM '2' OR request->>'expected_schema_version' IS DISTINCT FROM '20' OR
       request-ARRAY['contract_version','expected_schema_version','idempotency_key','binding_task_id','dispatch_policy_id','authorized_context']<>'{}'::jsonb OR
       COALESCE(length(request->>'idempotency_key'),0) NOT BETWEEN 1 AND 512 OR octet_length(request::text)>16384 THEN
        RAISE EXCEPTION 'invalid_complete_input_activation' USING ERRCODE='22023'; END IF;
    SELECT * INTO c FROM memoriesql.current_authorization_context();
    b:=memoriesql.complete_input_authorize(c.tenant_id,(request->>'binding_task_id')::uuid,(request->>'dispatch_policy_id')::uuid);
    ...
    IF NOT memoriesql.revisiting_context_valid(c.tenant_id,b.workspace_id,b.access_scope_id,b.package_id,request->'authorized_context')
      OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_dispatch_policies d WHERE d.tenant_id=c.tenant_id AND d.dispatch_policy_id=(request->>'dispatch_policy_id')::uuid AND d.execution_contract_revision=3)
    THEN RAISE EXCEPTION 'source_revisiting_qualification_or_context_unavailable' USING ERRCODE='42501'; END IF;
    request_hash:=encode(sha256(convert_to(request::text,'UTF8')),'hex');
    SELECT * INTO old FROM memoriesql.idempotency_receipts WHERE tenant_id=c.tenant_id AND operation_kind='complete_input.activate.v2' AND idempotency_key=request->>'idempotency_key';
    IF FOUND THEN
        IF old.request_hash<>request_hash THEN RAISE EXCEPTION 'idempotency_conflict' USING ERRCODE='23505'; END IF;
        RETURN old.response_receipt||'{"replayed":true}'::jsonb;
    END IF;
    -- Every acknowledged key owns a shared-ledger receipt, including natural replay.
    INSERT INTO memoriesql.idempotency_receipts(tenant_id,workspace_id,access_scope_id,idempotency_receipt_id,operation_kind,idempotency_key,request_hash,status,resource_kind,resource_id,attempt_count,created_at,updated_at)
    VALUES(c.tenant_id,c.workspace_id,b.access_scope_id,rid,'complete_input.activate.v2',request->>'idempotency_key',request_hash,'in_progress','semantic_task',b.task_id,1,started,started);
    SELECT * INTO e FROM memoriesql.complete_input_executions WHERE tenant_id=c.tenant_id AND binding_task_id=b.task_id;
    IF FOUND THEN
        IF e.execution_contract_revision<>3 OR e.authorized_context IS DISTINCT FROM request->'authorized_context' OR e.dispatch_policy_id<>(request->>'dispatch_policy_id')::uuid THEN RAISE EXCEPTION 'complete_input_activation_conflict' USING ERRCODE='23505'; END IF;
        SELECT response_receipt INTO result FROM memoriesql.idempotency_receipts WHERE tenant_id=c.tenant_id AND idempotency_receipt_id=e.activation_receipt_id;
        result:=result||jsonb_build_object('idempotency_receipt_id',rid,'replayed',true);
        PERFORM memoriesql.complete_input_authorize(c.tenant_id,b.task_id,(request->>'dispatch_policy_id')::uuid);
    ...
    IF original.status<>'policy_paused' OR original.pause_reason_code<>'complete_input_executor_unavailable' OR original.attempt_count<>0 OR original.cancel_requested_at IS NOT NULL THEN
        RAISE EXCEPTION 'complete_input_binding_not_transferable' USING ERRCODE='55000'; END IF;
    SELECT * INTO p FROM memoriesql.evidence_packages WHERE tenant_id=c.tenant_id AND package_id=b.package_id;
    INSERT INTO memoriesql.complete_input_executions VALUES(c.tenant_id,b.task_id,tid,(request->>'dispatch_policy_id')::uuid,(SELECT schema_version FROM memoriesql.source_objects WHERE tenant_id=c.tenant_id AND source_object_id=p.source_object_id),rid,3,request->'authorized_context');
    IF memoriesql.cancel_semantic_task(c.tenant_id,b.task_id,'complete_input.execution_transferred',clock_timestamp())<>'cancelled' THEN
        RAISE EXCEPTION 'complete_input_transfer_failed' USING ERRCODE='55000'; END IF;
    input:=jsonb_build_object('task_id',tid,'task_kind','memory.semantic.author-complete-unit','contract_revision',3,'target_reference',b.source_unit_id,'expected_target_revision',0,'requested_effort_key',NULL,'requested_budget',NULL,
      'evidence_manifest',jsonb_build_object('manifest_id','complete-input.'||tid::text,'revision',1,'references',jsonb_build_array(jsonb_build_object('reference_id',b.source_unit_id,'content_hash',p.inventory_hash,'declared_characters',p.character_count))),
    ...
    SELECT q.idempotency_receipt_id INTO eid FROM memoriesql.enqueue_semantic_task(tid,'complete-input.v2:'||b.task_id::text,'memoriesql.kernel','memory.semantic.author-complete-unit',3,'c8c172146c570bde75077745d7f00afa116b5db8f1b78bca4aebada561066e2d','semantic-tasks-v1:116ac71ae8ac94ad8abc3018a0577a60a5e5e543ddf3660ae7db467f09764aef',b.source_unit_id::text,0,input,input#>>'{evidence_manifest,manifest_id}',b.access_scope_id,started,b.task_id,started) q;
    result:=jsonb_build_object('contract_version',2,'authorized_context',request->'authorized_context','binding_task_id',b.task_id,'execution_task_id',tid,'idempotency_receipt_id',rid,'enqueue_receipt_id',eid,'package',b.package_pin,'replayed',false);
    PERFORM memoriesql.complete_input_authorize(c.tenant_id,b.task_id,(request->>'dispatch_policy_id')::uuid);
    ...
    RETURN result;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.activate_complete_input_v2(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION memoriesql.activate_complete_input_v2(jsonb) TO memoriesql_application;
...
BEGIN
    IF candidate->>'task_kind'='memory.semantic.author-complete-unit' AND candidate->>'contract_revision'='3' THEN
...
BEGIN
    ...
    IF task_record.target_kind = 'source_object' THEN
        ...
        END IF;
    END IF;
    IF task_record.task_kind='memory.semantic.author-complete-unit' AND task_record.contract_revision IN (2,3) THEN
        BEGIN
        ...
        END;
        IF task_record.contract_revision=3 THEN
...
BEGIN
    IF TG_TABLE_NAME='semantic_tasks' THEN
        IF NEW.task_kind<>'memory.semantic.author-complete-unit' OR NEW.contract_revision NOT IN (2,3) THEN RETURN NULL; END IF;
    ...
    IF e.execution_contract_revision=3 THEN
        expected:=jsonb_set(expected,'{contract_revision}','3');
    ...
    IF e.execution_task_id IS NULL OR b.task_id IS NULL OR t.task_id IS NULL OR t.input_payload IS DISTINCT FROM expected OR t.rerun_of_task_id IS DISTINCT FROM b.task_id OR
       ...
       t.task_contract_hash IS DISTINCT FROM (CASE WHEN e.execution_contract_revision=3 THEN 'c8c172146c570bde75077745d7f00afa116b5db8f1b78bca4aebada561066e2d' ELSE '5288a780557724d7c5be81afd28f50a0cc5e5b86a3c8e89ae241033e12f3f3fe' END) OR
       t.semantic_registry_hash IS DISTINCT FROM (CASE WHEN e.execution_contract_revision=3 THEN 'semantic-tasks-v1:116ac71ae8ac94ad8abc3018a0577a60a5e5e543ddf3660ae7db467f09764aef' ELSE 'semantic-tasks-v1:7e8a296cf1143deb86715144ed06cceefcb28dcc9955892ba7df5a06ded8f71f' END) THEN
...
BEGIN
    IF TG_TABLE_NAME='semantic_tasks' THEN
        ...
        ELSIF NEW.contract_revision IN (2,3) THEN
    ...
    ELSIF TG_TABLE_NAME='semantic_task_attempts' THEN
        ...
        IF t.task_kind='memory.semantic.author-complete-unit' AND (t.contract_revision NOT IN (2,3) OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_executions e WHERE e.tenant_id=t.tenant_id AND e.execution_task_id=t.task_id)) THEN
            RAISE EXCEPTION 'complete_input_executor_unavailable' USING ERRCODE='55000'; END IF;
    ELSE
        ...
        IF complete THEN
            ...
            WHERE r.tenant_id=NEW.tenant_id AND r.semantic_task_receipt_id=NEW.semantic_task_receipt_id AND i.operation_kind IN ('complete_input.apply.v1','complete_input.apply.v2');
            ...
            IF t.task_id IS NULL OR t.task_kind<>'memory.semantic.author-complete-unit' OR t.contract_revision NOT IN (2,3) OR t.status<>'running' OR t.cancel_requested_at IS NOT NULL OR
...
BEGIN
    is_complete := requested_command->>'contract_version' IN ('3','4');
    ...
    operation_key := CASE WHEN requested_command->>'contract_version'='4' THEN 'complete_input.apply.v2' WHEN is_complete THEN 'complete_input.apply.v1' WHEN is_correction THEN 'observation_correction.apply.v2'
    ...
    IF context_record.expires_at <= database_now
       ...
       OR NOT memoriesql.semantic_task_origin_authorized(
        ...
        RAISE EXCEPTION 'semantic annotation command is outside authorization'
            USING ERRCODE = '42501';
    END IF;
    IF requested_command->>'contract_version'='4' THEN
    ...
    INSERT INTO memoriesql.outbox_events (
        ...
        jsonb_build_object('contract_version', CASE WHEN requested_command->>'contract_version'='4' THEN 4 WHEN is_complete THEN 3 WHEN is_v2 THEN 2 ELSE 1 END), database_now, database_now
    );
...
CREATE OR REPLACE FUNCTION memoriesql.record_complete_input_exposure_v1(request_id uuid, payload_hash text, frame jsonb) RETURNS boolean
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
...
BEGIN
    ...
    IF e.execution_contract_revision<>2 OR c.principal_kind IS DISTINCT FROM 'service' OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_dispatch_policies d
...
INSERT INTO memoriesql.semantic_task_admission_policies (semantic_registry_hash,task_kind,contract_revision,owning_module,task_contract_hash,target_kind,required_capability,queue_name,base_priority,max_attempts,concurrency_key,concurrency_limit) VALUES ('semantic-tasks-v1:116ac71ae8ac94ad8abc3018a0577a60a5e5e543ddf3660ae7db467f09764aef','memory.semantic.author-complete-unit',3,'memoriesql.kernel','c8c172146c570bde75077745d7f00afa116b5db8f1b78bca4aebada561066e2d','canonical_semantics','memory.capture','capture',50,3,NULL,NULL);

CREATE FUNCTION memoriesql.authorize_source_delivery_v1(t uuid, task uuid, attempt uuid, generation bigint, worker text, instance text, package uuid) RETURNS boolean
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
BEGIN
 IF memoriesql.reauthorize_semantic_task(t,task,attempt,generation,worker,instance,'hydrate',clock_timestamp())<>'authorized' THEN
  RAISE EXCEPTION 'source_revisiting_stale_attempt' USING ERRCODE='42501'; END IF;
 PERFORM memoriesql.source_revisiting_authorize(t,task,package);
 IF memoriesql.reauthorize_semantic_task(t,task,attempt,generation,worker,instance,'hydrate',clock_timestamp())<>'authorized' THEN
  RAISE EXCEPTION 'source_revisiting_stale_attempt' USING ERRCODE='42501'; END IF;
 RETURN true;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.authorize_source_delivery_v1(uuid,uuid,uuid,bigint,text,text,uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION memoriesql.authorize_source_delivery_v1(uuid,uuid,uuid,bigint,text,text,uuid) TO memoriesql_worker;

CREATE FUNCTION memoriesql.read_source_evidence_v1(t uuid, task uuid, attempt uuid, generation bigint, worker text, instance text, request jsonb) RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
DECLARE result jsonb;
BEGIN
 PERFORM memoriesql.authorize_source_delivery_v1(t,task,attempt,generation,worker,instance,(request->>'package_id')::uuid);
 result:=memoriesql.source_revisiting_read(t,task,request);
 PERFORM memoriesql.authorize_source_delivery_v1(t,task,attempt,generation,worker,instance,(request->>'package_id')::uuid);
 IF octet_length(result::text)>262144 THEN RAISE EXCEPTION 'source_revisiting_response_bound' USING ERRCODE='54000'; END IF;
 RETURN result;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.read_source_evidence_v1(uuid,uuid,uuid,bigint,text,text,jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION memoriesql.read_source_evidence_v1(uuid,uuid,uuid,bigint,text,text,jsonb) TO memoriesql_worker;

CREATE FUNCTION memoriesql.record_source_delivery_v1(request_id uuid, payload_hash text, frame jsonb) RETURNS boolean
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
...
BEGIN
 ...
 IF c.principal_kind IS DISTINCT FROM 'service' OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_dispatch_policies d
  WHERE d.tenant_id=c.tenant_id AND d.dispatch_policy_id=e.dispatch_policy_id AND d.attestor_principal_id=c.principal_id
   AND d.status='active' AND d.expires_at>clock_timestamp() AND d.execution_contract_revision=3)
 ...
 SELECT * INTO a FROM memoriesql.semantic_task_attempts WHERE tenant_id=c.tenant_id AND attempt_id=i.attempt_id;
 PERFORM memoriesql.source_revisiting_authorize(c.tenant_id,i.task_id);
 IF c.principal_id=a.claimant_principal_id OR task.contract_revision<>3 OR task.status<>'running' OR a.status<>'running'
 ...
 RETURN true;
END; $$;
REVOKE ALL ON FUNCTION memoriesql.record_source_delivery_v1(uuid,text,jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION memoriesql.record_source_delivery_v1(uuid,text,jsonb) TO memoriesql_worker;

-- Legacy v1's ordered recording rule and receipts remain intact. Revision 3 uses actual set coverage.
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,memoriesql AS $$
 ...
 AND (e.execution_contract_revision<>3 OR (SELECT count(*)=(b.package_pin->>'required_parts')::integer FROM memoriesql.evidence_package_parts p WHERE p.tenant_id=t AND p.package_id=b.package_id))
 AND NOT EXISTS(
  SELECT 1 FROM memoriesql.evidence_package_parts p WHERE p.tenant_id=t AND p.package_id=b.package_id AND
   CASE WHEN e.execution_contract_revision=3 THEN NOT COALESCE(
