-- Forward authored-local mention acceptance. Published migrations remain unchanged.
-- Local uncertainty is immutable and separate from governed global resolution.
ALTER TABLE memoriesql.entity_mentions
 ADD COLUMN local_identity_state text,
 ADD COLUMN local_identity_reason text,
 ADD CONSTRAINT entity_mentions_local_identity CHECK (
  (local_identity_state IS NULL AND local_identity_reason IS NULL) OR
  (local_identity_state IS NOT NULL AND local_identity_state IN ('unresolved','ambiguous')
   AND char_length(surface_text) BETWEEN 1 AND 1024
   AND start_offset IS NULL AND end_offset IS NULL
   AND (local_identity_reason IS NULL OR
        (char_length(local_identity_reason) BETWEEN 1 AND 1024 AND local_identity_reason ~ '[^[:space:]]'))
   AND (local_identity_state <> 'ambiguous' OR local_identity_reason IS NOT NULL)));
CREATE TRIGGER accepted_entity_mention_guard BEFORE INSERT ON memoriesql.entity_mentions
...
ALTER TABLE memoriesql.complete_input_executions
 ...
 ADD CONSTRAINT complete_input_executions_execution_contract_revision_check CHECK(execution_contract_revision IN (2,3,4));
ALTER TABLE memoriesql.complete_input_dispatch_policies
 ...
 ADD CONSTRAINT complete_input_dispatch_polic_execution_contract_revision_check CHECK(execution_contract_revision IN (2,3,4));
...
BEGIN
 ...
 IF (e.execution_contract_revision IS NULL OR e.execution_contract_revision NOT IN (3,4)) OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_dispatch_policies d
...
CREATE OR REPLACE FUNCTION memoriesql.activate_complete_input_v3(request jsonb) RETURNS jsonb
LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path=pg_catalog,memoriesql SET lock_timeout='500ms' AS $$
...
BEGIN
    ...
    IF NOT memoriesql.revisiting_context_valid(c.tenant_id,b.workspace_id,b.access_scope_id,b.package_id,request->'authorized_context')
      OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_dispatch_policies d WHERE d.tenant_id=c.tenant_id AND d.dispatch_policy_id=(request->>'dispatch_policy_id')::uuid AND d.execution_contract_revision=4)
    ...
    SELECT * INTO old FROM memoriesql.idempotency_receipts WHERE tenant_id=c.tenant_id AND operation_kind='complete_input.activate.v3' AND idempotency_key=request->>'idempotency_key';
    IF FOUND THEN
    ...
    VALUES(c.tenant_id,c.workspace_id,b.access_scope_id,rid,'complete_input.activate.v3',request->>'idempotency_key',request_hash,'in_progress','semantic_task',b.task_id,1,started,started);
    SELECT * INTO e FROM memoriesql.complete_input_executions WHERE tenant_id=c.tenant_id AND binding_task_id=b.task_id;
    IF FOUND THEN
        IF e.execution_contract_revision<>4 OR e.authorized_context IS DISTINCT FROM request->'authorized_context' OR e.dispatch_policy_id<>(request->>'dispatch_policy_id')::uuid THEN RAISE EXCEPTION 'complete_input_activation_conflict' USING ERRCODE='23505'; END IF;
    ...
    INSERT INTO memoriesql.complete_input_executions VALUES(c.tenant_id,b.task_id,tid,(request->>'dispatch_policy_id')::uuid,(SELECT schema_version FROM memoriesql.source_objects WHERE tenant_id=c.tenant_id AND source_object_id=p.source_object_id),rid,4,request->'authorized_context');
...
REVOKE ALL ON FUNCTION memoriesql.activate_complete_input_v3(jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION memoriesql.activate_complete_input_v3(jsonb) TO memoriesql_application;
...
BEGIN
    IF candidate->>'task_kind'='memory.semantic.author-complete-unit' AND candidate->>'contract_revision' IN ('3','4') THEN
...
BEGIN
    ...
    IF task_record.target_kind = 'source_object' THEN
        ...
        END IF;
    END IF;
    IF task_record.task_kind='memory.semantic.author-complete-unit' AND task_record.contract_revision IN (2,3,4) THEN
        BEGIN
        ...
        END;
        IF task_record.contract_revision IN (3,4) THEN
...
BEGIN
    IF TG_TABLE_NAME='semantic_tasks' THEN
        IF NEW.task_kind<>'memory.semantic.author-complete-unit' OR NEW.contract_revision NOT IN (2,3,4) THEN RETURN NULL; END IF;
    ...
    IF e.execution_contract_revision IN (3,4) THEN
...
BEGIN
    IF TG_TABLE_NAME='semantic_tasks' THEN
        ...
        ELSIF NEW.contract_revision IN (2,3,4) THEN
    ...
    ELSIF TG_TABLE_NAME='semantic_task_attempts' THEN
        ...
        IF t.task_kind='memory.semantic.author-complete-unit' AND (t.contract_revision NOT IN (2,3,4) OR NOT EXISTS(SELECT 1 FROM memoriesql.complete_input_executions e WHERE e.tenant_id=t.tenant_id AND e.execution_task_id=t.task_id)) THEN
            RAISE EXCEPTION 'complete_input_executor_unavailable' USING ERRCODE='55000'; END IF;
    ELSE
        ...
        IF complete THEN
            ...
            WHERE r.tenant_id=NEW.tenant_id AND r.semantic_task_receipt_id=NEW.semantic_task_receipt_id AND i.operation_kind IN ('complete_input.apply.v1','complete_input.apply.v2','complete_input.apply.v3');
            ...
            IF t.task_id IS NULL OR t.task_kind<>'memory.semantic.author-complete-unit' OR t.contract_revision NOT IN (2,3,4) OR t.status<>'running' OR t.cancel_requested_at IS NOT NULL OR
...
BEGIN
    is_complete := requested_command->>'contract_version' IN ('3','4','5');
    ...
    operation_key := CASE WHEN requested_command->>'contract_version'='5' THEN 'complete_input.apply.v3' WHEN requested_command->>'contract_version'='4' THEN 'complete_input.apply.v2' WHEN is_complete THEN 'complete_input.apply.v1' WHEN is_correction THEN 'observation_correction.apply.v2'
    ...
    LOOP
        ...
        IF requested_command->>'contract_version'='5' THEN
    ...
    INSERT INTO memoriesql.outbox_events (
        ...
        jsonb_build_object('contract_version', CASE WHEN requested_command->>'contract_version'='5' THEN 5 WHEN requested_command->>'contract_version'='4' THEN 4 WHEN is_complete THEN 3 WHEN is_v2 THEN 2 ELSE 1 END), database_now, database_now
    );
...
BEGIN
 ...
 PERFORM memoriesql.source_revisiting_authorize(c.tenant_id,i.task_id);
 IF c.principal_id=a.claimant_principal_id OR task.contract_revision NOT IN (3,4) OR task.status<>'running' OR a.status<>'running'
...
LANGUAGE sql STABLE SECURITY DEFINER SET search_path=pg_catalog,memoriesql AS $$
 ...
 AND (e.execution_contract_revision NOT IN (3,4) OR (SELECT count(*)=(b.package_pin->>'required_parts')::integer FROM memoriesql.evidence_package_parts p WHERE p.tenant_id=t AND p.package_id=b.package_id))
 AND NOT EXISTS(
  SELECT 1 FROM memoriesql.evidence_package_parts p WHERE p.tenant_id=t AND p.package_id=b.package_id AND
   CASE WHEN e.execution_contract_revision IN (3,4) THEN NOT COALESCE(
